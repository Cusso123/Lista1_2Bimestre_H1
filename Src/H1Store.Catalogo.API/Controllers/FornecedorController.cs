...
namespace H1Store.Catalogo.API.Controllers
{
	...
	public class FornecedorController : ControllerBase
	{
		...
		{
			var fornecedores = await _fornecedorService.ObterTodosFornecedor();
		...
		{
			var fornecedor = await _fornecedorService.ObterFornecedorPorCodigo(id);
		...
		public IActionResult Put(int id, NovoFornecedorViewModel novoFornecedorViewModel)
		{
			...
			_fornecedorService.AtualizarFornecedor(novoFornecedorViewModel);
		...
		{
			bool excluidoComSucesso = _fornecedorService.RemoverFornecedor(id);
